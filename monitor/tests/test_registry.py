...
def test_get_protocol_config_returns_aave_v2_ethereum():
    """The one registered protocol/chain resolves to its config."""
    config = get_protocol_config("aave-v2-ethereum")
    assert config.protocol == "aave-v2"
    assert config.chain == "ethereum"


def test_get_risk_model_returns_aave_v2_ethereum():
    """The one registered protocol/chain resolves to its risk model."""
    model = get_risk_model("aave-v2-ethereum")
...
def test_unknown_protocol_key_raises_with_known_keys_listed():
    ...
    with pytest.raises(KeyError, match="aave-v2-ethereum"):
